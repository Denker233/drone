...
DfsStrategy::DfsStrategy(Vector3 position, Vector3 mid, Vector3 destination, const IGraph* graph) {
    ...
    std::vector<std::vector<float>> path_1 = graph->GetPath(start, midpoint, DepthFirstSearch::Default());
    std::vector<std::vector<float>> path_2 = graph->GetPath( midpoint, end, DepthFirstSearch::Default());
